...
import { activeTry, buttonLabel, finishTry } from '../vscode-extension/src/daemon.js';
...
beforeAll(async () => {
  ...
  if (!(await readDaemonAddress(env))) throw new Error('daemon did not start');
...
describe('the whole journey', () => {
  ...
  it('5–7. on the second change the learner takes over, and a tutorial opens', async () => {
    ...
    expect(opened).toContain('retry the original tool call');
  ...
  it('8–12. the retry parks, the button ends it, and the agent gets the learner’s code', async () => {
    ...
    // The tutorial has served its purpose.
  ...
  it('the journey left an honest record', async () => {
    ...
    // Explained up front both times — no denial was ever needed.
    expect(types.filter((t) => t === 'explain.shelved')).toHaveLength(2);
    expect(types.filter((t) => t === 'explain.prebound')).toHaveLength(2);
